...
Das verwendete MRT ist ein \emph{TIM Trio} der Firma \emph{Siemens, Erlangen, Deutschland}, welches ein Magnetfeld von $3\,T$ aufbauen kann.
...
Von beiden Versuchspersonen lagen 4 Blöcke in entsprechenden Versuchsbedingungen mit Oddball-Paradigma vor. Die Daten der 4 Blöcke wurden zusammen genommen und gemeinsam verrechnet. Dabei mussten die spezifischen Eigenschaften der Quellrekonstruktionsverfahren beachtet werden. Entsprechende Betrachtungen wurden in den folgenden Abschnitten \nameref{sec:lead-beam-mne} und \nameref{sec:amplitud} vorgenommen.
...
%%% Anzahl Trials: Gesamt, Standard, Deviant
%%% Dauer eines Blocks
...
%%% Unsicherheit im oberen Abschnitt
...
%%% Kommt das wirklich noch?

\subsection{Vergleichbarkeit der Einheiten}
\label{sec:amplitud}

Die Aktivität der Quellen aus den Rekonstruktionen von LCMV Beamformer und Minimum Norm Estimate unterscheiden sich in ihren Einheiten. Die Stärke der Dipole (Stromdipolmomente) wird in~$A \cdot m$ angegeben. Dies wird von Minimum Norm Estimate als solches berechnet und kann auch als solches interpretiert werden. Bei Beamformer werden die Aktivitäten durch den räumlichen Filter bestimmt. Dieser wirkt in drei Richtungen, entsprechend liegen drei Werte vor. Diese werden ebenfalls in~$A\cdot m$ angegeben. Es wird die Summe der Quadrate der drei Komponenten gebildet. Bei Beamformer ist die Interpretation der Aktivitäten jedoch schwierig, da es sich im Grunde genommen nur um eine Korrelation ???

%%% Hier noch auf Antwort warten.
...
%%% Bilder Frequenzanalyse noch durch Schwarz/Weiß und .eps austauschen
...
%  \captionsetup{justification=centering}
  ...
  \begin{subfigure}[c]{0.8\textwidth}
    \fbox{\includegraphics[width=\textwidth]{ergebnisse/frequenzanalye_raw.png}}
    \subcaption{Frequenzanalyse der Rohdaten}
    \label{img:freq-analy-raw}
  \end{subfigure}\vspace*{3mm}
  %
  \begin{subfigure}[c]{0.8\textwidth}
    \fbox{\includegraphics[width=\textwidth]{ergebnisse/frequenzanalye_ss.png}}
    \subcaption{Frequenzanalyse der SSS-Daten}
...
Als Rausch-Anteil wurden die ersten 100 ms gewählt. Dies entspricht der Baseline. Als Signal-Anteil wurde die MMN verwendet, welche im Bereich 150 ms bis 250 ms liegt. Damit ergeben sich zwei Zeitbereiche von je 100 ms, die in den Butterfly-Diagrammen der Rohdaten (siehe Abbildungen ???) dargestellt sind.
...
%  \captionsetup{justification=centering}
  ...
  \vspace*{3mm}
  \caption[Karte der Sensoren des \emph{Vectorview} MEG-Gerätes]{Karte der Sensoren des \emph{Vectorview} MEG-Gerätes in zweidimensionaler Darstellung. Im oberen Bereich liegt die Stirn, im unteren Bereich der Hinterkopf der Versuchsperson.}
  \label{img:sensmap}
\end{figure}
...
In Abbildung~\ref{img:pa07:aktiv} und~\ref{img:pa10:aktiv} sind die Aktivitätsdaten der rekonstruierten Quellen für die Versuchspersonen \texttt{pa07} und \texttt{pa10} abgebildet. Recht/Linksseitig (???). Deutlich wird, dass Minimum Norm Estimate (Abbildungen auf der rechten Seite) durchweg eine wesentlich konzentrierte Verteilung aufweisen, als LCMV Beamformer. Zusätzlich scheint Beamformer fälschlicherweise eine Aktivität auf der Gehirnoberseite festzustellen (Warum ???). Diese Aktivität sinkt jedoch mit zunehmender Verwendung rauschunterdrückender Verfahren. Daran wird deutlich, dass Beamformer offensichtlich große Probleme hat Rohdaten korrekt zu verarbeiten. Die These, dass Beamformer gut darin ist Rohdaten zu verarbeiten muss an dieser Stelle verworfen werden.

%%% Warum Aktivität auf der Gehirnoberseite?
